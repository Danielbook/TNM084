    ...
    while (!glfwWindowShouldClose(window))
    {
	    ...
	    for(i=0; i<IMAGE_SIZE; i++)
		{
		    ...
		    for(j=0; j<IMAGE_SIZE; j++)
			{
				...
				// Perlin noise
//				red = 128 + 127*noise3(2.0*x, 8.0*y, 0.5*time);
...
//				grn = 128 + 102*noise3(0*x, 8.0*y, 0.5*time);
//				blu = 128 + 102*noise3(10*x, 80.0*y, 5*time);;
//
//				k = (i + j*IMAGE_SIZE)*4;
//				pixels[k] = red;
//				pixels[k + 1] = grn;
...
//				pixels[k + 3] = grn;
                ...
                int baseColor;
                baseColor = (int) (100 + 52 * (1 + sin(y * 2 + time * 3)) * (0.3 + sin((30.0 * y + time * 2)) + sin(10.0 * (2.0 * y + time * 2)) / 16));

                baseColor += (4+sin(time))*1*snoise3(6.0*x, 6.0*y, time*2); // Add noise
                int waveHighlight = 225 + 70*snoise3(40*x, 60*y, 0.1*time);

                // Clamping
                baseColor = baseColor > 0 ? baseColor : 0;
                baseColor = baseColor < 255 ? baseColor : 255;
//                printf("\n%.3i", base);

                // Highlight of the wave
                float mult = baseColor/waveHighlight;
                waveHighlight = (int) (waveHighlight * (mult/mult));

//                printf("\n%.3f", mult);

                // Calculate the gradient to draw the white foamy parts of the waves
                k = (i + j*IMAGE_SIZE)*4;
                int prevY = (i+(j-4)*IMAGE_SIZE)*4;
                int yval = (baseColor-pixels[prevY+4])/4;
                int gradient = yval;

//                printf("\n%i", gradient);

                pixels[k] = (gradient > 0 ? waveHighlight : 0);
                pixels[k+1] = (gradient > 0 ? waveHighlight : 0);
                pixels[k+2] = baseColor;
                pixels[k + 3] = 255;
